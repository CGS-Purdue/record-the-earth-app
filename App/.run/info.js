...
const print_loop = (iterable) => {
  ...
  for (let item of items) {
    ...
    if (filter_item(item[0], '')) {
      formatted.push([item[0], item[1]]);
    }
  }
  ...
  formatted.forEach(function(item) {
    print_item(item[0], item[1]);
  });
  return;
};
const print_info = (name, data) => {
  print_title(name);
  print_loop(data);
};
...
const read_file_sync = (filepath) => {
  ...
  return fs.readFile(filepath, function read(err, data) {
    if (err) {
...
const command_list = ['app', 'config', 'env', 'paths', 'scripts'];
if (process.stdin.isTTY) {
  ...
  if (select && command_list.indexOf(select) >= 0) {
    switch (select) {
      case 'app':
        print_info(select, appconfig.expo);
        break;
      case 'config':
        print_info(select, mainpkg.config);
        break;
      case 'env':
        print_info(select, process.env);
        break;
      case 'scripts':
        print_info(select, mainpkg.scripts);
        break;
      case 'paths':
        print_info(select, mainpkg.dirs);
        break;
      default:
