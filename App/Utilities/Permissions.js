...
// If any permission resulted in a negative result, then that negative result is propagated here;
// that means top-level values are positive only if all component values are positive.
...
//              if any of has status !== 'granted' then that status is propagated here
...
async function checkPermissions(permKeyArray) {
  // let permTypeArray = [];
  // for (let perm of permKeyArray){ let permType = APP_PERMISSION[perm]; permTypeArray.push(permType); }

  let permTypeArray = permKeyArray.map((key) => { return APP_PERMISSION[key] });

  const { status, expires, permissions } = await Permissions.getAsync( ...permTypeArray );
...
export { checkPermissions, checkPermission, alertIfRemoteNotificationsDisabledAsync, requestPermissions };
